...
set warp_target to node_time - 5 - first_half_duration.
...
until done {
	...
	if nn:deltav:mag/max_acc < 2 set warp to 0. //warp
	else if nn:deltav:mag/max_acc < 4 set warp to 1.
	else if nn:deltav:mag/max_acc < 7 set warp to 2.
	else set warp to 3.
	
	if vang(facing:vector, steering:vector) > 1 { set th to 0. }
	else { set th to min(nn:deltav:mag/(max_acc*0.75), 1). }
